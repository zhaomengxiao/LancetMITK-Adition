...
#ifndef mitkGizmo_noscaleInteractor_h
#define mitkGizmo_noscaleInteractor_h
...
namespace mitk
{
  ...
  //!
  //! \sa Gizmo
  class MITKGIZMO_NOSCALE_EXPORT Gizmo_noscaleInteractor : public DataInteractor
  {
  public:
    mitkClassMacro(Gizmo_noscaleInteractor, DataInteractor);
    itkFactorylessNewMacro(Self);
    itkCloneMacro(Self);
    //const Vector3D &GetAxisOfRotation() const { return m_AxisOfRotation; }
      ...
      void SetGizmo_noscaleNode(DataNode *node);
  ...
  private:
    Gizmo_noscaleInteractor();
    ~Gizmo_noscaleInteractor() override;
    ...
    void MoveAlongAxis3D(StateMachineAction *, InteractionEvent *interactionEvent);
    ...
    void RotateAroundAxis3D(StateMachineAction *, InteractionEvent *interactionEvent);
    ...
    void ApplyTranslationToManipulatedObject3D(const Vector3D &projectedMovement);
    ...
    void ApplyRotationToManipulatedObject3D(double angle_deg);
    //! Pick a gizmo handle from a 2D event (passing by the 2D mapper)
    Gizmo_noscale::HandleType PickFrom2D(const InteractionPositionEvent *positionEvent);

    //! Pick a gizmo handle from a 3D event
    //! (passing by the general surface mapper and the gizmo object)
    Gizmo_noscale::HandleType PickFrom3D(const InteractionPositionEvent *positionEvent);

    void UpdateHandleHighlight();

    //! the Gizmo used for visual feedback and picking
    Gizmo_noscale::Pointer m_Gizmo_noscale;
    ...
    Gizmo_noscale::HandleType m_PickedHandle = Gizmo_noscale::NoHandle;

    //! Part of the gizmo that is currently highlighted
    Gizmo_noscale::HandleType m_HighlightedHandle = Gizmo_noscale::NoHandle;
    ...
    Point2D m_InitialGizmo_noscaleCenter2D; //< Initial position of the gizmo's center in screen coordinates
    Point3D m_InitialGizmo_noscaleCenter3D; //< Initial 3D position of the gizmo's center

    Vector3D m_AxisOfMovement; //< Axis along which we move when translating
    Vector3D m_AxisOfMovement3D;

	Vector3D m_AxisOfRotation; //< Axis around which we turn when rotating
    Vector3D m_AxisOfRotation3D; //< Axis around which we turn when rotating
