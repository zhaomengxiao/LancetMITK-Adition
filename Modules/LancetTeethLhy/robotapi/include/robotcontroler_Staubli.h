...
//#define ROBOT_SERVER_ADDRESS "192.168.1.254"
#define ROBOT_SERVER_ADDRESS "172.31.1.147"
...
//Cartesian coordinates of the point: (x,y,z,rx,ry,rz)
...
struct RobotCommandSendData
{
    ...
    {
        PointString = cmd_arg_point.arg(robot_point.x).arg(robot_point.y).arg(robot_point.z).arg(robot_point.rx).arg(robot_point.ry).arg(robot_point.rz);
    }
    ...
    void set_tcpinfo_string()
    {
        TcpString = cmd_arg_tcpinfo.arg(robot_tcp.tcptype).arg(robot_tcp.cog.weight)
                ...
                .arg(robot_tcp.tcp.rx).arg(robot_tcp.tcp.ry).arg(robot_tcp.tcp.rz);
    }
...
public slots:
    ...
    void setspeed(int s);                   // percent 0-100
