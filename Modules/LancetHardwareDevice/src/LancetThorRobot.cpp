...
LancetThorRobot::LancetThorRobot()
{
	this->SetRobotIpAddress("192.168.10.75");
}

void LancetThorRobot::Connect()
{
...
void LancetThorRobot::Disconnect()
{
...
void LancetThorRobot::PowerOn()
{
	releaseBrake();
}

void LancetThorRobot::PowerOff()
{
	holdBrake();
}

void LancetThorRobot::Translate(double x, double y, double z)
{
...
void LancetThorRobot::Translate(double* aDirection, double aLength)
{
	Translate(aDirection[0] * aLength, aDirection[1] * aLength, aDirection[2] * aLength);
}

void LancetThorRobot::Rotate(double* aDirection, double aAngles)
{
	vtkSmartPointer<vtkTransform> transform = vtkSmartPointer<vtkTransform>::New();
	vtkSmartPointer<vtkMatrix4x4> vtkMatrix = vtkSmartPointer<vtkMatrix4x4>::New();
	vtkMatrix->DeepCopy(this->GetBaseToTCP());
...
void LancetThorRobot::RecordInitialPos()
{
	m_initJoints = this->GetJointAngles();
}

void LancetThorRobot::GoToInitialPos()
{
...
void LancetThorRobot::SetTCPToFlange()
{
...
bool LancetThorRobot::SetPositionMode()
{
	int ret = changeControlMode(T_MODE_POSITION);
	return ret < 0 ? false : true;
}

bool LancetThorRobot::SetJointImpendanceMode()
{
	int ret = changeControlMode(T_MODE_JOINT_IMPEDANCE);
	return ret < 0 ? false : true;
}

bool LancetThorRobot::SetCartImpendanceMode()
{
	int ret = changeControlMode(T_MODE_CART_IMPEDANCE);
	return ret < 0 ? false : true;
}

std::vector<double> LancetThorRobot::GetJointAngles()
{
...
void LancetThorRobot::SetJointAngles(std::vector<double> aJointAngles)
{
...
vtkSmartPointer<vtkMatrix4x4> LancetThorRobot::GetBaseToTCP()
{
	double pose[6] = {};
	getTcpPos(pose, m_IpAddress);
...
vtkSmartPointer<vtkMatrix4x4> LancetThorRobot::GetFlangeToTCP()
{
...
vtkSmartPointer<vtkMatrix4x4> LancetThorRobot::GetBaseToFlange()
{
	double joints[7] = { 0.0 };
...
void LancetThorRobot::RobotTransformInBase(double* aMatrix)
{
	double pose[6] = {};
	vtkSmartPointer<vtkMatrix4x4> vtkMatrix = vtkSmartPointer<vtkMatrix4x4>::New();
...
void LancetThorRobot::RobotTransformInTCP(double* aMatrix)
{
	double pose[6] = {};
	vtkSmartPointer<vtkMatrix4x4> TBase2Tcp = vtkSmartPointer<vtkMatrix4x4>::New();
	TBase2Tcp->DeepCopy(this->GetBaseToTCP());
...
std::vector<double> LancetThorRobot::GetCartDampParams()
{
	double arrStiff[6] = {};
	double dblDamp = 0;
...
bool LancetThorRobot::SetCartDampParams(std::vector<double> aDampParams)
{
...
std::vector<double> LancetThorRobot::GetCartStiffParams()
{
	double arrStiff[6] = {};
	double dblDamp = 0;
	std::vector<double> ret;
...
bool LancetThorRobot::SetCartStiffParams(std::vector<double> aStiffParams)
{
...
std::vector<double> LancetThorRobot::GetCartImpeda()
{
...
bool LancetThorRobot::SetCartImpeda(std::vector<double> aImpeda)
{
	double arrstiff[6] = {};
	double dblDamp = 0;
	for (int i = 0; i < 6; ++i)
...
bool LancetThorRobot::SetVelocity(double aVelocity)
{
	return false;
}
