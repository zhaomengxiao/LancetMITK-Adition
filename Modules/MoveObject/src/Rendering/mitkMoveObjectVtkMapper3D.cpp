...
namespace mitk
{
  class MoveObjectVtkMapper3D::Impl
  {
...
mitk::MoveObjectVtkMapper3D::Impl::LocalStorage::LocalStorage()
...
void mitk::MoveObjectVtkMapper3D::SetDefaultProperties(DataNode *node, BaseRenderer *renderer, bool overwrite)
{
  Superclass::SetDefaultProperties(node, renderer, overwrite);
}

mitk::MoveObjectVtkMapper3D::MoveObjectVtkMapper3D() : m_Impl(new Impl)
{
}

mitk::MoveObjectVtkMapper3D::~MoveObjectVtkMapper3D()
{
  delete m_Impl;
}

void mitk::MoveObjectVtkMapper3D::ApplyColorAndOpacityProperties(BaseRenderer*, vtkActor*)
{
  //Superclass::ApplyColorAndOpacityProperties(renderer, actor);
}

void mitk::MoveObjectVtkMapper3D::ApplyMoveObjectProperties(BaseRenderer *renderer, vtkActor *actor)
{
  if (actor == nullptr)
    return;
...
void mitk::MoveObjectVtkMapper3D::GenerateDataForRenderer(BaseRenderer *renderer)
{
  auto dataNode = this->GetDataNode();
  ...
  if (needGenerateData)
  {
    ...
    this->ApplyMoveObjectProperties(renderer, localStorage->Actor);

    this->ApplyColorAndOpacityProperties(renderer, localStorage->HandleActor);
    this->ApplyMoveObjectProperties(renderer, localStorage->HandleActor);

    this->ApplyColorAndOpacityProperties(renderer, localStorage->SelectedHandleActor);
    this->ApplyMoveObjectProperties(renderer, localStorage->SelectedHandleActor);
...
vtkProp *mitk::MoveObjectVtkMapper3D::GetVtkProp(BaseRenderer *renderer)
{
