...
void CameraTest::CreateQtPartControl(QWidget *parent)
{
  ...
  //connect(m_Controls.pushButton_test, &QPushButton::clicked, this, &CameraTest::TestCalculateRepeatability);
...
void CameraTest::CalculateRepeatability()
{
	...
	for (int i = 0; i < size(m_ProbeRealTimePose_list); i++)
	{
		double Di = sqrt(pow((x_average - m_ProbeRealTimePose_list[i][0]), 2) +
			pow((y_average - m_ProbeRealTimePose_list[i][1]), 2) +
			pow((z_average - m_ProbeRealTimePose_list[i][2]), 2));
		cout << "Di" << Di << endl;
		deviation_list.push_back(Di);
	}
	double deviation_average;
	double deviation_sum = 0;
	for (int i = 0; i < size(deviation_list); i++)
	{
		deviation_sum += deviation_list[i];
	}
	deviation_average = deviation_sum / size(deviation_list);
	cout << "deviation_average: " << deviation_average << endl;
...
{
	m_ProbeRealTimePose_list = { {33.9407,105.136,1170.93},{33.898,105.079,1170.66},{33.9731,105.11,1170.89},
	{33.935,105.134,1170.9}, {33.8835,105.073,1170.8},{33.9285,105.174,1171.05} };
	double x_sum = 0;
	double y_sum = 0;
	double z_sum = 0;
	for (int i = 0; i < size(m_ProbeRealTimePose_list); i++)
