...
namespace lancet
{
	...
	bool LANCET_STATEMACHINE_SERVICES_PLUGIN
		plugin::IsLancetModuleFormat(const QString& id)
	{
		return QRegExp("^org_mitk_lancet_[a-z_\\.]*$").exactMatch(id);
	}
			...
			const QString& stateId, int type)
	{
		...
		{
			if (dir.dirName() != plugin::GetMitkPluginProductName(stateId))
			{
