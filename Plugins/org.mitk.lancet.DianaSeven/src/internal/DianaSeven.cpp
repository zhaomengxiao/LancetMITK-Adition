...
{
	// 获取,probe的index, 并获取当前时间点的 NavigationData
	auto probeIndex = m_VegaToolStorage->GetToolIndexByName("Probe");
	...
	// 计算Fprobe到Fprobe_new的变换矩阵
	vtkSmartPointer<vtkMatrix4x4> probeToprobeNew = vtkSmartPointer<vtkMatrix4x4>::New();
	vtkMatrix4x4::Invert(m_ndiToprobe, probeToprobeNew); // 计算 m_ndiToprobe 的逆矩阵并存储在 resultMatrix 中
	//MITK_INFO << "m_ndiToprobe";
	//m_ndiToprobe->Print(std::cout);

	vtkMatrix4x4::Multiply4x4(probeToprobeNew, m_currentProbeMatrix, probeToprobeNew); // 计算逆矩阵与 m_ndiToprobe_new 的乘积并更新 resultMatrix
	//MITK_INFO << "m_ndiToprobe_new";
	//probeToprobeNew->Print(std::cout);
	vtkMatrix4x4::Multiply4x4(m_init_robotMatrix, probeToprobeNew, probeToprobeNew); // 计算逆矩阵与 T_robot 的乘积并更新 resultMatrix
