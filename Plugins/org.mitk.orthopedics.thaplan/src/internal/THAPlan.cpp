...
{
	if (!initPelvis() || !initFemurL() || !initFemurR() || !initCup() || !initStem())
	{
		return;
	}
	m_Reduction = othopedics::Reduction::New();
	m_Reduction->SetPelvis(m_Pelvis);
	m_Reduction->SetFemur_L(m_Femur_L);
	m_Reduction->SetFemur_R(m_Femur_R);
	m_Reduction->SetCup(m_Cup);
	m_Reduction->SetStem(m_Stem);

	m_Reduction->m_OperationSide = m_OperationSide;
	...
	// auto nodeset = GetDataStorage()->GetAll();
...
void THAPlan::onPushButton_cupPlan_clicked()
{
	...
	//show pelvis and cup
	GetDataStorage()->GetNamedNode("pelvis")->SetVisibility(true);
	GetDataStorage()->GetNamedNode("Cup_54")->SetVisibility(true);
...
void THAPlan::onPushButton_redeuction_clicked()
{
	//hide all node,make it clear
	HideAllNode(GetDataStorage());
...
bool THAPlan::initCup()
{
	m_Cup = othopedics::Cup::New();
	m_Cup->m_Side = othopedics::ESide::left;
	m_dn_cup = GetDataStorage()->GetNamedNode("Cup_54");
...
bool THAPlan::initStem()
{
	...
	m_Stem->SetLandMark(othopedics::ELandMarks::stem_CutPoint, p1);
	m_Stem->SetLandMark(othopedics::ELandMarks::stem_NeckCanalIntersectPoint, p2);
	...
	m_dn_stem = GetDataStorage()->GetNamedNode("Stem_4");
	if (m_dn_stem == nullptr)
	{
...
void THAPlan::CreateQtPartControl(QWidget *parent)
{
  ...
  connect(m_Controls.pushButton_reduction, &QPushButton::clicked, this, &THAPlan::onPushButton_redeuction_clicked);
}
